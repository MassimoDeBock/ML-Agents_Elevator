...
public class ElevatorAgent_Two : ElevatorAgent
{
    ...
    [SerializeField] private BufferSensorComponent m_PriorityBuffer;
    ...
    protected override void ExtraObservations()
    {
        List<List<float>> destinations = new List<List<float>>();
        for (int i = 0; i < elevator.building.floors.Count; i++)
        {
    ...
    private void AddPriorityInfo()
    {
        ...
        foreach (FloorPiece floor in elevator.building.floors)
        {
            int count = floor.PriorityPassengerCount();
            if (count > maxWaiting)
            {
                maxWaiting = count;
            }
        ...
        foreach (PassengerLogic passenger in elevator.passengers)
        {
            if (!passenger.HasPriority()) continue;
            //add 1 to the destination floor count
            passengers[passenger.destinationFloor] += 1;
        ...
        for (int i = 0; i < elevator.building.floors.Count; i++)
        {
            ...
            m_PriorityBuffer.AppendObservation(new float[] { waiting[i] / maxWaiting, passengers[i] / maxDestination, elevator.building.floors[i].GetTimeNormalized(), maxWaitingTimerEachFloor[i] });
        }
    }
}
